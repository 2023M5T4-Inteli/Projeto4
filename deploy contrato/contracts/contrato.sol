...
assim como a taxa administrativa*/
    constructor(uint256 _adminTax, address[] memory _members, string[] memory _imeis, uint _lmiTax) {
    ...
    function inverseCompare(string memory str1, string memory str2) public pure returns (bool){
    ...
    function indemnity(address payable _userAddress, string memory _imei, uint256 _indemnity) public onlyOwner isActive(_userAddress) {
        require(
            compare(memberImei[_userAddress], _imei),
            "IMEI fornecido nao corresponde"
        );
        if (memberFunds[_userAddress]* lmiTax >= _indemnity){
            _userAddress.transfer(_indemnity);
            memberFunds[_userAddress] -= _indemnity; // recalcula o fundo do segurado
        }
        else {
            _indemnity -= memberFunds[_userAddress];
            memberFunds[_userAddress] -= _indemnity;
            uint valorRemanecente = _indemnity - memberFunds[_userAddress];
            uint valorTotal;
        for (uint x =0 ; x<members.length; x++){
            if (members[x] != _userAddress){
                valorTotal += memberFunds[members[x]];
            }
        }
        for (uint256 i = 0; i < members.length; i++){
                if(members[i] != _userAddress){
                    uint porcentagem = memberFunds[members[i]] / valorTotal;
                    memberFunds[members[i]] -= _valorRemanecente * porcentagem;
                }
        }
    }

    // Regra de negócio: A Coover pode retirar a taxa administrativa a qualquer momento
    function adminWithdrawal() external onlyOwner {
